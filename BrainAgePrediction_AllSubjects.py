...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 20 15:49:44 2026
...
OUT_DIR = os.path.join(os.environ["WORK"], "ines/results/results_with_bias_correction_all_subjects")
...
# === Select healthy subjects onlyfrom scipy.stats import linregress
...
finalmodel_path = os.path.join(
    os.environ["WORK"],
    "ines/results/results_with_bias_correction/model_trained_on_all_healthy_corrected.pt"
)

print("Loading model from:", finalmodel_path)
...
metrics_path = os.path.join(OUT_DIR, "model_performance_metrics.txt")

with open(metrics_path, "w") as f:
    f.write("===== MODEL PERFORMANCE (AD-DECODE) =====\n")
...
output_path = os.path.join(OUT_DIR, "brain_age_predictions_all_subjects_bias_corrected.csv")
...
slope, intercept, r_value, p_value, std_err = linregress(df_preds["Age"], df_preds["BAG"])
r2_bag = r_value**2
plt.title(f"BAG vs Age\nR² = {r2_bag:.3f} | p = {p_value:.3e}")
plt.xlabel("Chronological Age")
plt.ylabel("Brain Age Gap (BAG)")
plt.legend()
plt.tight_layout()
plt.show()
plt.savefig(os.path.join(OUT_DIR, "plot_BAG_vs_Age.png"), dpi=200, bbox_inches="tight")
...
plt.axhline(0, linestyle="--", color="gray")
# === Compute regression stats ===
slope, intercept, r_value, p_value, std_err = linregress(df_preds["Age"], df_preds["cBAG"])
r2_cbag = r_value**2
plt.title(f"cBAG vs Age\nR² = {r2_cbag:.3f} | p = {p_value:.3e}")
plt.xlabel("Chronological Age")
plt.ylabel("Corrected Brain Age Gap (cBAG)")
plt.legend()
plt.tight_layout()
plt.show()
plt.savefig(os.path.join(OUT_DIR, "plot_cBAG_vs_Age.png"), dpi=200, bbox_inches="tight")
