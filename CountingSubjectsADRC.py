...
m["Sex_Label"] = m[COL_SEX].map({
    1: "Male (M)",
    2: "Female (F)",
    "1": "Male (M)",
    "2": "Female (F)"
}).fillna(m[COL_SEX].astype(str))

# === SEX ===
print("=== SEX ===")
if "Sex_Label" not in m.columns:
    print("NA (missing column)\n")
else:
    sx = m["Sex_Label"].dropna().astype(str)
    total = len(sx)
    if total == 0:
        print("NA\n")
    else:
...
def normalize_apoe(x):
    ...
    if len(s) == 2:
        a1, a2 = sorted([s[0], s[1]])
        return f"APOE{a1}{a2}"
    return s
m["APOE_norm"] = m[COL_APOE].apply(normalize_apoe)

# === APOE GENOTYPE ===
print("=== APOE GENOTYPE ===")
if "APOE_norm" not in m.columns:
    print("NA (missing column)\n")
else:
    ap = m["APOE_norm"].dropna().astype(str)
    total = len(ap)
    if total == 0:
        print("NA\n")
    else:
