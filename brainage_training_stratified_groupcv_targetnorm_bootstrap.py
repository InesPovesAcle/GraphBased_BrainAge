...
"""
Brain-age training for graph sets generated by graph_builder_with_normcog_feature_sets.py.

The graph builder already saves one graph set per feature-set option:
    .../graphs/imaging_only/
    .../graphs/imaging_demographics/
    .../graphs/imaging_biomarkers/
    .../graphs/full/
    .../graphs/full_no_cardiovascular/

This script loops over those feature-set folders, loads the corresponding
healthy-control graphs for training/CV, trains the GNN, and optionally applies
the final model to the corresponding all-subject graph set.

Important: this script does NOT mask graph.global_features again. Each loaded
.pt file already contains the correct global_features for its feature set.
"""
...
COHORT = "AD_DECODE" #"ADRC" #"ADNI" # "HABS"
...
EXPERIMENT_TAG = "stratified_groupcv_targetnorm"
...
def run_one_feature_set(feature_set):
    ...
    if BOOTSTRAP_CI:
        ...
        bootstrap_summary_df = pd.concat([bs_raw, bs_bc], ignore_index=True)
    ...
    bc_ckpt["bias_correction"] = {"a": float(a_global), "b": float(b_global)}
...
def main():
    ...
    for feature_set in RUN_FEATURE_SETS:
        ...
        if os.path.exists(p):
            bootstrap_frames.append(pd.read_csv(p))
    if bootstrap_frames:
    ...
    print("\n" + "=" * 80)
    print("ALL ABLATION MODELS COMPLETED")
