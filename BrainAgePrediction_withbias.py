...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 17 11:09:57 2026
...
#MODEL
    ...
    # Batch norm​
    # Concat true, heads 8​
    # Patience 40 ​que hace este codigo:
...
import re  # For extracting numcv_start_time = time.perf_counter()erical IDs using regular expressions
import time
import torch
import random
import numpy as np
...
# ---- Silence common noisy warnings ----
warnings.filterwarnings("ignore", category=FutureWarning)     # torch.load pickle warning
warnings.filterwarnings("ignore", category=RuntimeWarning)    # numpy mean of empty slice
warnings.filterwarnings("ignore", category=UserWarning)       # sklearn / torch minor warnings
...
'''
OUT_DIR = os.path.join(os.environ["WORK"], "ines/results/results_with_bias_correction")
...
def compute_nodewise_clustering_coefficients(matrix):
    ...
    Parameters:
        matrix (pd.DataFrame): 84x84 connectivity matrixque hace este codigo:
...
import torch
torch.save(graph_data_list_addecode, os.path.join(OUT_DIR, "graph_data_list_addecode.pt"))
...
all_train_losses = []   # [repeat][fold] -> list of epoch losses
all_test_losses  = []   # [repeat][fold] -> list of epoch losses
all_early_stopping_epochs = []  # (repeat, fold, early_epoch)
...
for repeat in range(repeats):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
        print(f"\n--- Repeat {repeat+1}/{repeats} | Fold {fold+1}/{k} ---")

        train_data = [graph_data_list_addecode[i] for i in train_idx]
        test_data  = [graph_data_list_addecode[i] for i in test_idx]

        # seed SOLO para entrenamiento (no afecta a splits)
        train_seed = 1000 + base_seed + repeat*100 + fold
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                ...
                ckpt = f"model_rep_{repeat+1}_fold_{fold+1}.pt"
        ...
        all_early_stopping_epochs.append((repeat+1, fold+1, early_stop_epoch))
        repeat_train_losses.append(train_losses)
        repeat_test_losses.append(test_losses)

    all_train_losses.append(repeat_train_losses)
    all_test_losses.append(repeat_test_losses)

cv_end_time = time.perf_counter()
cv_seconds = cv_end_time - cv_start_time
h = int(cv_seconds // 3600)
m = int((cv_seconds % 3600) // 60)
s = cv_seconds % 60
print(f"\n✅ Total time CV ({repeats} repeats × {k} folds): {h:02d}:{m:02d}:{s:06.2f}")
...
for repeat in range(repeats):
    for fold in range(k):
        plt.plot(all_train_losses[repeat][fold],
                 label=f"Train - Rep {repeat+1} Fold {fold+1}",
                 linestyle="dashed", alpha=0.35)
        plt.plot(all_test_losses[repeat][fold],
                 label=f"Test - Rep {repeat+1} Fold {fold+1}",
                 alpha=0.35)

plt.xlabel("Epochs")
plt.ylabel("Smooth L1 Loss")
plt.title("Learning Curve (All Repeats × Folds)")
plt.grid(True)
# OJO: la leyenda con 70 líneas será enorme. Mejor comentarla:
...
for epoch in range(epochs):
    ...
    for repeat in range(repeats):
        for fold in range(k):
            if epoch < len(all_train_losses[repeat][fold]):
                epoch_train.append(all_train_losses[repeat][fold][epoch])
                epoch_test.append(all_test_losses[repeat][fold][epoch])
...
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("Learning Curve (Mean ± Std Across All Repeats/Folds)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
all_bias_slope = []
all_bias_intercept = []
coef_rows = []   # one row per fold×repeat (slope/intercept)
...
fold_mae, fold_rmse, fold_r2 = [], [], []
all_y_true, all_y_pred = [], []
all_subject_ids, fold_tags, repeat_tags = [], [], []

all_bag, all_cbag = [], []
all_bias_slope, all_bias_intercept = [], []
coef_rows = []

for repeat in range(REPEATS):
    split_seed = base_seed + repeat
    skf_eval = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=split_seed)

    for fold, (train_idx, test_idx) in enumerate(skf_eval.split(graph_data_list_addecode, age_bins)):
        print(f"\n--- Evaluating Rep {repeat+1}/{REPEATS} | Fold {fold+1}/{N_FOLDS} ---")
        ...
        # load trained model
        model = BrainAgeGATv2(global_feat_dim=16).to(device)
        ckpt_path = f"model_rep_{repeat+1}_fold_{fold+1}.pt"
        state_dict = torch.load(ckpt_path, map_location=device)
        model.load_state_dict(state_dict)
        model.eval()

        # ---- Predict TRAIN (fit bias on TRAIN only)
        ...
        with torch.no_grad():
            for batch in train_loader:
                ...
                y_true_train.extend(trues.cpu().tolist())

        age_train = np.array(y_true_train, dtype=float)
        bag_train = np.array(y_pred_train, dtype=float) - age_train

        reg = LinearRegression().fit(age_train.reshape(-1, 1), bag_train)
        slope = float(reg.coef_[0])
        intercept = float(reg.intercept_)

        coef_rows.append({"Fold": fold+1, "Repeat": repeat+1, "Slope": slope, "Intercept": intercept})

        # ---- Predict TEST + apply correction
        ...
        bag_test = np.array(y_pred, dtype=float) - age_test
        cbag_test = bag_test - reg.predict(age_test.reshape(-1, 1))
        ...
        repeat_tags.extend([repeat+1] * len(y_true))

        all_bag.extend(bag_test.tolist())
        all_cbag.extend(cbag_test.tolist())
        all_bias_slope.extend([slope] * len(y_true))
        all_bias_intercept.extend([intercept] * len(y_true))

print("\n================== FINAL METRICS AD-DECODE ==================")
print(f"Global MAE:  {np.mean(fold_mae):.2f} ± {np.std(fold_mae):.2f}")
print(f"Global RMSE: {np.mean(fold_rmse):.2f} ± {np.std(fold_rmse):.2f}")
print(f"Global R²:   {np.mean(fold_r2):.2f} ± {np.std(fold_r2):.2f}")
print("=============================================================\n")
...
df_preds_addecode = pd.DataFrame({
    ...
    "BAG":             all_bag,
    "cBAG":            all_cbag,
    "Bias_Slope":      all_bias_slope,
    "Bias_Intercept":  all_bias_intercept,
    "Fold":            fold_tags,
    "Repeat":          repeat_tags
})
...
df_bias_coefs = pd.DataFrame(coef_rows)
coef_path = os.path.join(OUT_DIR, "bias_coefficients_per_fold_repeat.csv")
df_bias_coefs.to_csv(coef_path, index=False)
print(f"Bias coefficients saved to: {coef_path}")


# ==========================
# BIAS SUMMARY (pretty print)  + subject-level check  (FIXED)
# ==========================
import os
import pandas as pd
import numpy as np
...
df_preds = pd.read_csv(os.path.join(results_dir, "cv_predictions_addecode.csv"))
df_bias  = pd.read_csv(os.path.join(results_dir, "bias_coefficients_per_fold_repeat.csv"))

# --- Make column names robust ---
df_bias = df_bias.rename(columns={
    "Slope": "slope",
    "Intercept": "intercept",
    "SLOPE": "slope",
    "INTERCEPT": "intercept"
...
# --- Age column robust ---
if "Age" not in df_preds.columns:
    if "Real_Age" in df_preds.columns:
        df_preds["Age"] = df_preds["Real_Age"]
    else:
        raise KeyError("No encuentro la columna de edad: esperaba 'Real_Age' o 'Age'.")

# --- Ensure BAG/cBAG exist ---
if "BAG" not in df_preds.columns:
    if "Predicted_Age" in df_preds.columns:
        df_preds["BAG"] = df_preds["Predicted_Age"] - df_preds["Age"]
    else:
        raise KeyError("No encuentro 'Predicted_Age' para calcular BAG")

if "cBAG" not in df_preds.columns:
    df_preds["cBAG"] = df_preds["BAG"]

# --- Aggregate slope/intercept (fold-wise, solo informativo) ---
if ("slope" not in df_bias.columns) or ("intercept" not in df_bias.columns):
    raise KeyError(f"En df_bias no están slope/intercept. Columnas: {df_bias.columns.tolist()}")

slope_mean = df_bias["slope"].mean()
slope_std  = df_bias["slope"].std()
inter_mean = df_bias["intercept"].mean()
inter_std  = df_bias["intercept"].std()

# ------------------------------------------------------------
# (A) RAW correlations (all rows = fold×repeat predictions)
# ------------------------------------------------------------
corr_bag_raw  = df_preds["Age"].corr(df_preds["BAG"])
corr_cbag_raw = df_preds["Age"].corr(df_preds["cBAG"])

# ------------------------------------------------------------
# (B) SUBJECT-LEVEL correlations (1 row per subject)
#     This is the correct way to judge bias in repeated CV.
# ------------------------------------------------------------
df_subj = (
    df_preds
...
# === Global bias fit on OUT-OF-FOLD (subject-level) predictions ===
# Fit BAG ~ Age on subject-mean OOF predictions
reg_g = LinearRegression().fit(df_subj[["Age"]], df_subj["BAG"])
slope_g = float(reg_g.coef_[0])
intercept_g = float(reg_g.intercept_)

# Corrected (global) brain-age gap
df_subj["cBAG_global"] = df_subj["BAG"] - reg_g.predict(df_subj[["Age"]])

corr_bag_subj   = df_subj["Age"].corr(df_subj["BAG"])
corr_cbag_subj  = df_subj["Age"].corr(df_subj["cBAG"])
corr_cbag_glob  = df_subj["Age"].corr(df_subj["cBAG_global"])

print("\n=========== GLOBAL (Oof, subject-level) ===========")
print(f"slope_global: {slope_g:+.3f}")
print(f"intercept_global: {intercept_g:+.3f}")
print(f"SUBJ corr(Age, BAG)        : {corr_bag_subj:+.3f}")
print(f"SUBJ corr(Age, cBAG_global): {corr_cbag_glob:+.3f}  ")
print("===================================================\n")

# ------------------------------------------------------------
# (C) Propagate cBAG_global back to EVERY row (fold×repeat)
#     so CSV + any downstream plots use the correct correction.
# ------------------------------------------------------------
map_cbag_global = df_subj.set_index("Subject_ID")["cBAG_global"]
df_preds["cBAG_global"] = df_preds["Subject_ID"].map(map_cbag_global)

# Save updated CSV (overwrites) so you don’t keep mixing versions
out_csv = os.path.join(results_dir, "cv_predictions_addecode.csv")
df_preds.to_csv(out_csv, index=False)




# Recompute correlations using the correct cBAG_global
corr_cbag_global_raw  = df_preds["Age"].corr(df_preds["cBAG_global"])
df_subj2 = (
    df_preds
    .groupby("Subject_ID", as_index=False)
    .agg(
        Age=("Age", "first"),
        BAG=("BAG", "mean"),
        cBAG_global=("cBAG_global", "mean"),
    )
)
corr_cbag_global_subj = df_subj2["Age"].corr(df_subj2["cBAG_global"])

print("\n================ BRAIN AGE BIAS =================")
print(f"Mean slope (BAG~Age): {slope_mean:.3f} ± {slope_std:.3f}")
print(f"Mean intercept:       {inter_mean:.3f} ± {inter_std:.3f}\n")

print(f"RAW  corr(Age, BAG)         : {corr_bag_raw:+.3f}")
print(f"RAW  corr(Age, cBAG)        : {corr_cbag_raw:+.3f}")
print(f"RAW  corr(Age, cBAG_global) : {corr_cbag_global_raw:+.3f}\n")

print(f"SUBJ corr(Age, BAG)         : {corr_bag_subj:+.3f}")
print(f"SUBJ corr(Age, cBAG)        : {corr_cbag_subj:+.3f}")
print(f"SUBJ corr(Age, cBAG_global) : {corr_cbag_global_subj:+.3f}")

if abs(corr_cbag_global_subj) < 0.05:
    print("✔ Bias removed (subject-level, global OOF correction)")
elif abs(corr_cbag_global_subj) < 0.10:
    print("~ Small residual bias (subject-level, global OOF correction)")
else:
    print("✘ Bias still present (subject-level, global OOF correction)")
...
textstr = f"MAE: {np.mean(fold_mae):.2f} ± {np.std(fold_mae):.2f}\nR²: {np.mean(fold_r2):.2f} ± {np.std(fold_r2):.2f}"
...
# ==========================================================
# CV LEARNING CURVES (Train and Validation/Test) with 95% CI
# Saves 2 separate figures in OUT_DIR
# Uses: all_train_losses, all_test_losses, OUT_DIR, CV_EPOCHS
# ==========================================================
import os
import numpy as np
import matplotlib.pyplot as plt
...
# --- safety checks ---
assert "OUT_DIR" in globals(), "OUT_DIR no está definido todavía."
assert len(all_train_losses) > 0 and len(all_test_losses) > 0, "No hay pérdidas del CV (all_train_losses/all_test_losses)."

n_repeats = len(all_train_losses)
n_folds   = len(all_train_losses[0])

def mean_ci_over_runs(all_losses, max_epochs):
    means, stds, ns = [], [], []
    ...
    for ep in range(max_epochs):
        ...
        for r in range(n_repeats):
            for f in range(n_folds):
                if ep < len(all_losses[r][f]):
                    vals.append(all_losses[r][f][ep])

        vals = np.array(vals, dtype=float)
        ns.append(vals.size)
        means.append(vals.mean() if vals.size else np.nan)
        stds.append(vals.std(ddof=1) if vals.size > 1 else np.nan)

    means = np.array(means)
    stds  = np.array(stds)
    ns    = np.array(ns)

    sem = stds / np.sqrt(np.maximum(ns, 1))
    ci95 = 1.96 * sem

    valid = (ns > 0) & np.isfinite(means)
    last = np.where(valid)[0][-1] + 1

    x = np.arange(1, last + 1)
    return x, means[:last], ci95[:last]


# ---- TRAIN ----
x_tr, train_mean, train_ci = mean_ci_over_runs(all_train_losses, CV_EPOCHS)

plt.figure(figsize=(8,5))
plt.plot(x_tr, train_mean, label="Train mean")
plt.fill_between(x_tr, train_mean-train_ci, train_mean+train_ci, alpha=0.25)
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("CV Train Loss (Mean ± 95% CI)")
plt.grid(True)
plt.tight_layout()

train_path = os.path.join(OUT_DIR, "cv_train_loss_mean_95CI.png")
plt.savefig(train_path, dpi=300, bbox_inches="tight")
plt.close()
print("Saved:", train_path)

# ---- VALIDATION / TEST ----
x_va, val_mean, val_ci = mean_ci_over_runs(all_test_losses, CV_EPOCHS)

plt.figure(figsize=(8,5))
plt.plot(x_va, val_mean, label="Validation/Test mean")
plt.fill_between(x_va, val_mean-val_ci, val_mean+val_ci, alpha=0.25)
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("CV Validation/Test Loss (Mean ± 95% CI)")
plt.grid(True)
plt.tight_layout()

val_path = os.path.join(OUT_DIR, "cv_validation_loss_mean_95CI.png")
...
torch.save(final_model.state_dict(), os.path.join(OUT_DIR, "model_trained_on_all_healthy_corrected.pt"))
print("\nFinal model saved as 'model_trained_on_all_healthy_corrected.pt'")

# ==========================================================
# RESULTS SUMMARY: subject-level (clean) + performance + stability
# Saves clean tables + metric tables + plots into OUT_DIR
# ==========================================================
import os
import numpy as np
import pandas as pd
...
# CONFIG: paths
# -------------------------
# Use your existing OUT_DIR if it exists; otherwise set manually

pred_path = os.path.join(OUT_DIR, "cv_predictions_addecode.csv")
if not os.path.exists(pred_path):
    # If your CSV is somewhere else, change this path
    raise FileNotFoundError(f"No encuentro el archivo: {pred_path}")

df = pd.read_csv(pred_path)

# -------------------------
# Basic column robustness
# -------------------------
...
if "Age" not in df.columns:
    if "Real_Age" in df.columns:
        ...
        raise KeyError("No encuentro Age/Real_Age en el CSV.")

# Make sure BAG exists
if "BAG" not in df.columns:
...
# -------------------------
# If cBAG_global is missing, compute it (OOF subject-level)
# -------------------------
if "cBAG_global" not in df.columns:
    # subject-level OOF means
    df_subj_tmp = (
    ...
    reg_g = LinearRegression().fit(df_subj_tmp[["Age"]], df_subj_tmp["BAG"])
    df_subj_tmp["cBAG_global"] = df_subj_tmp["BAG"] - reg_g.predict(df_subj_tmp[["Age"]])

    # map back to all rows
    map_cbag = df_subj_tmp.set_index("Subject_ID")["cBAG_global"]
    df["cBAG_global"] = df["Subject_ID"].map(map_cbag)

    # overwrite CSV so you keep one consistent version
    df.to_csv(pred_path, index=False)
    print("Added cBAG_global and overwrote:", pred_path)

# ==========================================================
# 1) CLEAN SUBJECT-LEVEL RESULTS (one row per subject)
# ==========================================================
# You will get mean prediction per subject (across repeats),
# plus variability across repeats (std) -> useful stability per subject
agg_dict = {
    "Age": ("Age", "first"),
    "Predicted_Age_mean": ("Predicted_Age", "mean"),
    "Predicted_Age_std": ("Predicted_Age", "std"),
    "BAG_mean": ("BAG", "mean"),
    "BAG_std": ("BAG", "std"),
    "cBAG_global_mean": ("cBAG_global", "mean"),
    "cBAG_global_std": ("cBAG_global", "std"),
    "n_predictions": ("Predicted_Age", "size"),
}
...
# Optional: if you still have cBAG (fold-wise) in the CSV, summarize it too
if "cBAG" in df.columns:
    df_subject2 = (
        df.groupby("Subject_ID", as_index=False)
          .agg(cBAG_mean=("cBAG", "mean"),
               cBAG_std=("cBAG", "std"))
    )
    df_subject = df_subject.merge(df_subject2, on="Subject_ID", how="left")
subject_out = os.path.join(OUT_DIR, "subject_level_predictions_clean.csv")
df_subject.to_csv(subject_out, index=False)
print("Saved subject-level clean results:", subject_out)

# ==========================================================
# AUC: APOE4 carrier (yes/no) usando subject-level scores
# ==========================================================
from sklearn.metrics import roc_auc_score, roc_curve

# 1) Construir etiqueta APOE4 desde metadata original (string)
# OJO: aquí usamos addecode_healthy_metadata_pca (ya lo tienes en memoria)
# pero necesitas el genotype ORIGINAL, no el label-encoded.
# Si en addecode_healthy_metadata_pca perdiste el string, usa df_metadata_PCA_healthy_withConnectome
# que venía del merge antes del LabelEncoder.
if "genotype" in df_metadata_PCA_healthy_withConnectome.columns:
    geno_src = df_metadata_PCA_healthy_withConnectome[["MRI_Exam_fixed", "genotype"]].copy()
else:
    raise KeyError("No encuentro genotype string en df_metadata_PCA_healthy_withConnectome")

geno_src["genotype_str"] = geno_src["genotype"].astype(str).str.lower()
geno_src["APOE4"] = geno_src["genotype_str"].str.contains("4").astype(int)

# --- FIX: igualar tipos/formatos de ID para el merge ---
df_subject["Subject_ID"] = df_subject["Subject_ID"].astype(str).str.zfill(5)
geno_src["MRI_Exam_fixed"] = geno_src["MRI_Exam_fixed"].astype(str).str.zfill(5)
geno_src = geno_src.drop_duplicates(subset="MRI_Exam_fixed")  # por si acaso

# 2) Merge con df_subject (1 fila por sujeto)
df_auc = df_subject.merge(geno_src[["MRI_Exam_fixed", "APOE4"]],
                          left_on="Subject_ID", right_on="MRI_Exam_fixed", how="inner")

print("df_subject N:", len(df_subject))
print("geno_src N unique:", geno_src["MRI_Exam_fixed"].nunique())
print("df_auc N merged:", len(df_auc))

score_col = "cBAG_global_mean"   # recomendado
y = df_auc["APOE4"].values
s = df_auc[score_col].values

# 3) AUC + ROC curve
if y.min() == y.max():
    print("No puedo calcular AUC APOE4: solo hay una clase en los datos.")
else:
    auc_apoe4 = roc_auc_score(y, s)
    fpr, tpr, _ = roc_curve(y, s)
    print(f"\nROC-AUC APOE4 (score={score_col}): {auc_apoe4:.3f} | N={len(df_auc)} | Pos={int(y.sum())}")

    plt.figure(figsize=(6, 6))
    plt.plot(fpr, tpr, label=f"AUC={auc_apoe4:.3f}")
    plt.plot([0, 1], [0, 1], linestyle="--")
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC: APOE4 carrier vs non-carrier")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()

    out_png = os.path.join(OUT_DIR, "roc_auc_apoe4.png")
    plt.savefig(out_png, dpi=300, bbox_inches="tight")
    plt.close()
    print("Saved ROC plot:", out_png)

    # Guardar también en CSV por si quieres reportarlo
    out_txt = os.path.join(OUT_DIR, "roc_auc_apoe4.txt")
    with open(out_txt, "w") as f:
        f.write(f"AUC_APOE4={auc_apoe4:.6f}\n")
        f.write(f"Score={score_col}\n")
        f.write(f"N={len(df_auc)} Pos={int(y.sum())}\n")
    print("Saved AUC summary:", out_txt)

# ==========================================================
# 2) MODEL PERFORMANCE (recommended: subject-level OOF mean)
# ==========================================================
...
y_true_subj = df_subject["Age"].values
y_pred_subj = df_subject["Predicted_Age_mean"].values

perf = {
    "Level": ["Subject-OOF-Mean"],
    "MAE": [mean_absolute_error(y_true_subj, y_pred_subj)],
    "RMSE": [rmse(y_true_subj, y_pred_subj)],
    "R2": [r2_score(y_true_subj, y_pred_subj)],
    "N_subjects": [len(df_subject)],
}

df_perf = pd.DataFrame(perf)
perf_out = os.path.join(OUT_DIR, "performance_summary_subject_level.csv")
df_perf.to_csv(perf_out, index=False)
print("Saved performance summary:", perf_out)

# ==========================================================
# 3) STABILITY (metrics per fold×repeat + summary)
# ==========================================================
# Compute MAE/RMSE/R2 for each fold×repeat (using rows in that split)
# Note: this uses per-row predictions; it is useful for stability across models.
if ("Fold" in df.columns) and ("Repeat" in df.columns):
    rows = []
    for (fold, rep), g in df.groupby(["Fold", "Repeat"]):
        yt = g["Age"].values
        yp = g["Predicted_Age"].values
        rows.append({
            "Fold": int(fold),
            "Repeat": int(rep),
    ...
    df_fr = pd.DataFrame(rows).sort_values(["Fold", "Repeat"])
    fr_out = os.path.join(OUT_DIR, "metrics_per_fold_repeat.csv")
    df_fr.to_csv(fr_out, index=False)
    print("Saved fold×repeat metrics:", fr_out)

    # Stability summary (mean ± std across models)
    stab = {
        "Metric": ["MAE", "RMSE", "R2"],
        "Mean": [df_fr["MAE"].mean(), df_fr["RMSE"].mean(), df_fr["R2"].mean()],
        "Std":  [df_fr["MAE"].std(),  df_fr["RMSE"].std(),  df_fr["R2"].std()],
        "Min":  [df_fr["MAE"].min(),  df_fr["RMSE"].min(),  df_fr["R2"].min()],
        "Max":  [df_fr["MAE"].max(),  df_fr["RMSE"].max(),  df_fr["R2"].max()],
    }
    df_stab = pd.DataFrame(stab)
    stab_out = os.path.join(OUT_DIR, "stability_summary_fold_repeat.csv")
    df_stab.to_csv(stab_out, index=False)
    print("Saved stability summary:", stab_out)

else:
    print("Fold/Repeat columns not found -> skipping fold×repeat stability tables.")

# ==========================================================
# 4) NICE PLOTS (optional but useful)
# ==========================================================

# (A) Subject-level Predicted vs Real
plt.figure(figsize=(6, 6))
plt.scatter(df_subject["Age"], df_subject["Predicted_Age_mean"], alpha=0.7, edgecolors="k")
mn = float(min(df_subject["Age"].min(), df_subject["Predicted_Age_mean"].min()))
mx = float(max(df_subject["Age"].max(), df_subject["Predicted_Age_mean"].max()))
plt.plot([mn, mx], [mn, mx], linestyle="--")
plt.xlabel("Real Age")
plt.ylabel("Predicted Age (OOF mean per subject)")
plt.title("Subject-level: Predicted vs Real")
plt.grid(True)
plt.tight_layout()
plot1 = os.path.join(OUT_DIR, "plot_subject_level_pred_vs_real.png")
plt.savefig(plot1, dpi=300)
plt.close()
print("Saved plot:", plot1)

# (B) cBAG_global distribution (subject-level)
plt.figure(figsize=(7, 4))
plt.hist(df_subject["cBAG_global_mean"].values, bins=20, edgecolor="black")
plt.xlabel("cBAG_global (mean per subject)")
plt.ylabel("Count")
plt.title("Subject-level cBAG_global distribution")
plt.grid(True)
plt.tight_layout()
plot2 = os.path.join(OUT_DIR, "plot_subject_level_cBAG_global_hist.png")
plt.savefig(plot2, dpi=300)
plt.close()
print("Saved plot:", plot2)

# (C) If fold×repeat metrics exist, plot MAE distribution
if os.path.exists(os.path.join(OUT_DIR, "metrics_per_fold_repeat.csv")):
    df_fr = pd.read_csv(os.path.join(OUT_DIR, "metrics_per_fold_repeat.csv"))
    plt.figure(figsize=(7, 4))
    plt.hist(df_fr["MAE"].values, bins=15, edgecolor="black")
    plt.xlabel("MAE (per fold×repeat model)")
    plt.ylabel("Count")
    plt.title("Stability: MAE distribution across folds×repeats")
    plt.grid(True)
    plt.tight_layout()
    plot3 = os.path.join(OUT_DIR, "plot_stability_mae_hist.png")
    plt.savefig(plot3, dpi=300)
    plt.close()
    print("Saved plot:", plot3)

print("\n✅ Done: clean subject-level results + performance + stability saved in OUT_DIR")


plt.figure(figsize=(8,5))
plt.plot(range(1, epochs+1), final_train_losses)
plt.xlabel("Epoch")
plt.ylabel("Smooth L1 Loss")
plt.title("Final model training loss")
plt.grid(True)
plt.tight_layout()
loss_plot_path = os.path.join(OUT_DIR, "final_model_loss_curve.png")
...
csv_path = os.path.join(OUT_DIR, "final_model_loss_history.csv")
df_final_hist.to_csv(csv_path, index=False)
print("Saved loss history to:", csv_path)

print("Saved: final_model_loss_history.csv")
