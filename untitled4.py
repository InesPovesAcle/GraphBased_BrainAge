...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 16 15:06:24 2026
...
def bias_correct_preds(y_true_train, y_pred_train, y_pred_to_correct):
    """
    Fit ONLY on training:
    y_pred_train = a + b*y_true_train
    Then correct predictions:
    y_corr = (y_pred - a) / b
    """
    y_true_train = np.asarray(y_true_train).reshape(-1, 1)
    y_pred_train = np.asarray(y_pred_train).reshape(-1, 1)
    y_pred_to_correct = np.asarray(y_pred_to_correct).reshape(-1, 1)

    lr = LinearRegression()
    lr.fit(y_true_train, y_pred_train)
    a = float(lr.intercept_[0])
    b = float(lr.coef_[0, 0])

    if abs(b) < 1e-8:
        return y_pred_to_correct.ravel(), (a, b)

    y_corr = (y_pred_to_correct - a) / b
    return y_corr.ravel(), (a, b)
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    test_data = [graph_data_list_addecode[i] for i in test_idx]
    test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)
    train_loader_eval = DataLoader(train_data, batch_size=batch_size, shuffle=False)
    ...
    for rep in range(repeats_per_fold):
        ...
        # ---- 1) obtener predicciones en TRAIN (para calcular bias) ----
        y_true_train = []
        y_pred_train = []
        ...
        with torch.no_grad():
            for data in train_loader_eval:
                data = data.to(device)
                output = model(data).view(-1)
                y_pred_train.extend(output.cpu().tolist())
                y_true_train.extend(data.y.view(-1).cpu().tolist())
        ...
        with torch.no_grad():
            for data in test_loader:
                data = data.to(device)
                output = model(data).view(-1)
                y_pred_test.extend(output.cpu().tolist())
                y_true_test.extend(data.y.view(-1).cpu().tolist())
                
              

        
        # ---- 3) bias correction (fit SOLO en train) ----
        y_pred_corrected, (a, b) = bias_correct_preds(
            y_true_train, y_pred_train, y_pred_test
        )
        
        # ---- 4) métricas usando predicción corregida ----
        mae = mean_absolute_error(y_true_test, y_pred_corrected)
        r2 = r2_score(y_true_test, y_pred_corrected)
        
        repeat_maes.append(mae)
        repeat_r2s.append(r2)
        
        all_y_true.extend(y_true_test)
        all_y_pred.extend(y_pred_corrected)
...
# =============================
# Fit bias correction on healthy training set
# =============================
final_model.eval()
final_loader_eval = DataLoader(graph_data_list_addecode, batch_size=6, shuffle=False)

y_true_h = []
y_pred_h = []

with torch.no_grad():
    for data in final_loader_eval:
        data = data.to(device)
        out = final_model(data).view(-1)
        y_pred_h.extend(out.cpu().tolist())
        y_true_h.extend(data.y.view(-1).cpu().tolist())

_, (a_final, b_final) = bias_correct_preds(y_true_h, y_pred_h, y_pred_h)

print("Final bias parameters:", a_final, b_final)

np.savez(os.path.join(OUT_DIR, "bias_params_healthy.npz"), a=a_final, b=b_final)
print("Saved bias params to:", os.path.join(OUT_DIR, "bias_params_healthy.npz"))
