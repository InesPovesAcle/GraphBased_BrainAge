...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 16 16:41:05 2026
...
def train(model, train_loader, optimizer, criterion):
    ...
    for data in train_loader:
        ...
        # weight por muestra (si existe); si no, weight=None
        w = getattr(data, "sample_weight", None)
        if w is not None:
            w = w.view(-1).to(output.device)
        loss = criterion(output, data.y.view(-1), weight=w)
        loss.backward()
        optimizer.step()
...
# ==========================================================
# AGE-REWEIGHTING (training only) + WEIGHTED SmoothL1Loss
# ==========================================================
import numpy as np
import torch
import torch.nn as nn

def _make_age_bins(y, n_bins=5):
    """
    y: np.array of ages
    returns bin_id per sample (0..n_bins-1) using quantiles
    """
    # quantile cut -> balanced bins (robusto a distribución)
    qs = np.quantile(y, np.linspace(0, 1, n_bins + 1))
    qs[0] -= 1e-6
    qs[-1] += 1e-6
    bin_id = np.digitize(y, qs[1:-1], right=True)
    return bin_id.astype(int), qs

def attach_train_only_age_weights(train_data, n_bins=5, power=1.0, clamp=(0.5, 3.0)):
    """
    Calcula pesos SOLO con train_data y los guarda en cada Data como .sample_weight
    - inverse freq por bin (con potencia)
    - normaliza para que mean(weight)=1
    """
    y_train = np.array([d.y.item() for d in train_data], dtype=float)
    bin_id, _ = _make_age_bins(y_train, n_bins=n_bins)
    ...
    # normaliza mean=1 (importante para estabilidad)
    w = w / np.mean(w)

    # clamp opcional para evitar pesos extremos
    if clamp is not None:
        w = np.clip(w, clamp[0], clamp[1])

    for d, wi in zip(train_data, w):
        d.sample_weight = torch.tensor([float(wi)], dtype=torch.float)
...
class WeightedSmoothL1Loss(nn.Module):
    ...
    def forward(self, pred, target, weight=None):
        # pred/target: [batch]
        base = torch.nn.functional.smooth_l1_loss(
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    attach_train_only_age_weights(train_data, n_bins=5, power=1.0, clamp=(0.5, 3.0))
    ...
    for repeat in range(repeats_per_fold):
        ...
        criterion = WeightedSmoothL1Loss(beta=1)
...
print(f"\n✅ Total time for training CV (7 folds × {repeats_per_fold} repeats): {h:02d}:{m:02d}:{s:06.2f} (hh:mm:ss)")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
    ...
    for rep in range(repeats_per_fold):
        ...
        state_dict = torch.load(
            f"model_fold_{fold+1}_rep_{rep+1}.pt",
            weights_only=True
        )
...
train_loader = DataLoader(
    [graph_data_list_addecode[i] for i in train_idx],
    batch_size=BATCH_SIZE, shuffle=False
)
...
def predict_loader(model, loader, device):
    ...
    return np.array(y_true, float), np.array(y_pred, float), subj_ids

    # ----- 1) predict TRAIN -----
    y_tr, p_tr, _ = predict_loader(model, train_loader, device)
    
    # Fit: p = a*y + b (TRAIN ONLY)
    lr = LinearRegression()
    lr.fit(y_tr.reshape(-1, 1), p_tr.reshape(-1, 1))
    a = float(lr.coef_.ravel()[0])
    b = float(lr.intercept_.ravel()[0])
    
    # ----- 2) predict TEST -----
    y_true, y_pred, subj_ids = predict_loader(model, test_loader, device)
    
    # ----- 3) apply correction on TEST -----
    eps = 1e-8
    y_pred_corr = (y_pred - b) / (a + eps)
    
    # Usa y_pred_corr en vez de y_pred a partir de aquí:
    # métricas
    mae  = mean_absolute_error(y_true, y_pred_corr)
    from sklearn.metrics import root_mean_squared_error
    rmse = root_mean_squared_error(y_true, y_pred_corr)
    r2   = r2_score(y_true, y_pred_corr)
    
    # y para guardar en CSV:
    y_pred_to_save = y_pred_corr.tolist()
    ...
    all_y_pred.extend(y_pred_to_save)
...
repeats_per_fold = len(all_train_losses[0])

def mean_ci_over_runs(all_losses, max_epochs):
    """
    all_losses: list[fold][rep] -> list of losses per epoch (variable length due to early stopping)
    returns: x (1..last), mean, ci95
    """
    means, stds, ns = [], [], []
    ...
    for ep in range(max_epochs):
        ...
        for f in range(k_folds):
            for r in range(repeats_per_fold):
                if ep < len(all_losses[f][r]):
                    vals.append(all_losses[f][r][ep])
    ...
    # recorta hasta último epoch con datos reales
    valid = (ns > 0) & np.isfinite(means)
    last = np.where(valid)[0][-1] + 1  # slicing
