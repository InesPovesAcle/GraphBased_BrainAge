...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  2 19:42:17 2026
...
results_dir = os.path.join(os.environ["WORK"], "ines/results/BrainAgePredictionADNI")
...
for root, _, files in os.walk(base_path_adni):
    for fn in files:
        if not fn.endswith("_conn_plain.csv"):
            continue

        stem = fn.replace("_conn_plain.csv", "")  # R####_y#
        m = re.match(r"^R(\d+)_y(\d+)$", stem)
        if not m:
            bad += 1
            continue

        connectome_key = stem  # ej: "R4288_y0"
        file_path = os.path.join(root, fn)

        try:
            df = pd.read_csv(file_path, header=None)
            adni_connectomes[connectome_key] = df
        except Exception as e:
            print(f"Error loading {file_path}: {e}")

print("Total ADNI connectomes loaded:", len(adni_connectomes))
if bad:
    print("Skipped (unexpected filename pattern):", bad)
...
metadata_path_adni = os.path.join(os.environ["WORK"], "ines/data/harmonization/ADNI/metadata/FINAL_idaSearch_3_19_2025.xlsx")
...
df_adni_metadata["RID"] = (
    df_adni_metadata["Subject ID"].astype(str)
    .str.extract(r"_(\d+)$")[0]
    .astype(float)
    .astype("Int64")
)
...
visit_map = {
    ...
    # si tienes más visitas que quieras incluir, añádelas aquí
}

df_adni_metadata["Visit_Clean"] = df_adni_metadata["Visit"].map(visit_map)
df_adni_metadata = df_adni_metadata.dropna(subset=["RID", "Visit_Clean"]).copy()

df_adni_metadata["connectome_key"] = (
    "R" + df_adni_metadata["RID"].astype(int).astype(str) + "_" + df_adni_metadata["Visit_Clean"]
)
...
heatmap_path = os.path.join(results_dir, f"heatmap_{connectome_key}.png")
...
fa_path = os.path.join(os.environ["WORK"],"ines/data/Regional_stats/ADNI/ADNI_studywide_stats_for_fa.txt")
...
vol_path = os.path.join(os.environ["WORK"],"ines/data/Regional_stats/ADNI/ADNI_studywide_stats_for_volume.txt")
...
def threshold_connectome(matrix, percentile=85):
    """
...
for subject, matrix in matched_connectomes_healthy_adni.items():
    ...
    thresholded_matrix = threshold_connectome(matrix, percentile=85)
...
# === Save
save_path = os.path.join(results_dir, "graph_data_list_adni.pt")

torch.save(graph_data_list_adni, save_path)
...
class BrainAgeGATv2_BiasCorrected(torch.nn.Module):
    """
    Wrapper que devuelve Predicted_Age_corrected usando:
    cBAG = (pred_raw - age) - (slope*age + intercept)
    pred_corr = age + cBAG
    """
    def __init__(self, base_model: torch.nn.Module, slope: float, intercept: float):
        super().__init__()
        self.base = base_model
        self.register_buffer("slope", torch.tensor(float(slope)))
        self.register_buffer("intercept", torch.tensor(float(intercept)))

    def forward(self, data):
        pred_raw = self.base(data).view(-1)  # [batch]

        # En inferencia NECESITAS la edad cronológica.
        # Puedes usar data.y (como ahora) o crear data.age_chrono.
        if hasattr(data, "age_chrono"):
            age = data.age_chrono.view(-1).to(pred_raw.device)
        else:
            age = data.y.view(-1).to(pred_raw.device)

        bag = pred_raw - age
        cbag = bag - (self.slope * age + self.intercept)
        pred_corr = age + cbag
        return pred_corr
    ...
    for repeat in range(repeats_per_fold):
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                patience_counter = 0
                ckpt_path = os.path.join(ckpt_dir, f"model_adni1_fold_{fold+1}_rep_{repeat+1}.pt")
...
secs = cv_end - cv_start
h = int(secs // 3600)
m = int((secs % 3600) // 60)
s = secs % 60
print(f"\n✅ Total CV time (folds×repeats): {h:02d}:{m:02d}:{s:06.2f}")

with open(os.path.join(results_dir, "timing_cv.txt"), "w") as f:
    f.write(f"Total_CV_seconds={secs:.3f}\n")
    f.write(f"Total_CV_hms={h:02d}:{m:02d}:{s:06.2f}\n")
...
plt.grid(True)
plt.tight_layout()
lc_path = os.path.join(results_dir, "learning_curve_mean_std.png")
...
##############################################################
# EVALUATION + BIAS CORRECTION (fit on TRAIN folds only)
##############################################################
from sklearn.metrics import mean_absolute_error, r2_score
from torch_geometric.loader import DataLoader

REPEATS = repeats_per_fold
N_FOLDS = k
...
for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_adni, age_bins)):
    ...
    train_loader = DataLoader([graph_data_list_adni[i] for i in train_idx],
                              batch_size=BATCH_SIZE, shuffle=False)
    test_loader  = DataLoader([graph_data_list_adni[i] for i in test_idx],
                              batch_size=BATCH_SIZE, shuffle=False)
    ...
    for rep in range(REPEATS):
        ...
        model = BrainAgeGATv2(global_feat_dim=16).to(device)
        ckpt_path = os.path.join(ckpt_dir, f"model_adni1_fold_{fold+1}_rep_{rep+1}.pt")
...
df_preds_adni = pd.DataFrame(all_rows)
pred_csv = os.path.join(results_dir, "cv_predictions_adni_bias_corrected.csv")
df_preds_adni.to_csv(pred_csv, index=False)
print(f"Saved predictions CSV: {pred_csv}")

# ==========================
# GLOBAL OOF BIAS CORRECTION (subject-level)
# ==========================
import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression

dfp = df_preds_adni.copy()

# 1) Subject-level mean across repeats/folds rows
df_subj = (
    dfp.groupby("Subject_ID", as_index=False)
       .agg(
           Age=("Real_Age", "first"),
           Pred_raw=("Predicted_Age_raw", "mean"),
       )
)

# 2) Fit global bias on OOF subject-level predictions
df_subj["BAG"] = df_subj["Pred_raw"] - df_subj["Age"]
reg_g = LinearRegression().fit(df_subj[["Age"]], df_subj["BAG"])

df_subj["cBAG_global"] = df_subj["BAG"] - reg_g.predict(df_subj[["Age"]])
df_subj["Pred_corr_global"] = df_subj["Age"] + df_subj["cBAG_global"]

# 3) Map back to every row (so CSV/plots use one consistent correction)
map_cbag = df_subj.set_index("Subject_ID")["cBAG_global"]
map_pred = df_subj.set_index("Subject_ID")["Pred_corr_global"]

dfp["cBAG_global"] = dfp["Subject_ID"].map(map_cbag)
dfp["Predicted_Age_corrected_global"] = dfp["Subject_ID"].map(map_pred)

# 4) Correlations (subject-level is the meaningful one)
corr_bag_subj = df_subj["Age"].corr(df_subj["BAG"])
corr_cbag_glob_subj = df_subj["Age"].corr(df_subj["cBAG_global"])

print("\n=== GLOBAL OOF (subject-level) bias check ===")
print(f"slope_global: {float(reg_g.coef_[0]):+.3f}")
print(f"intercept_global: {float(reg_g.intercept_):+.3f}")
print(f"SUBJ corr(Age, BAG)        = {corr_bag_subj:+.3f}")
print(f"SUBJ corr(Age, cBAG_global)= {corr_cbag_glob_subj:+.3f}")

# ==========================
# SAVE GLOBAL BIAS PARAMS
# ==========================
bias_params = {
    "slope_global": float(reg_g.coef_[0]),
    "intercept_global": float(reg_g.intercept_),
}
bias_path = os.path.join(results_dir, "bias_correction_global.pt")
torch.save(bias_params, bias_path)
print("Saved global bias params:", bias_path)




# 5) Save updated CSV (overwrite or new file)
out_csv = os.path.join(results_dir, "cv_predictions_adni_bias_corrected_GLOBAL_OOF.csv")
dfp.to_csv(out_csv, index=False)
print("Saved global-OOF corrected CSV:", out_csv)




df_bias = pd.DataFrame(coef_rows)
coef_csv = os.path.join(results_dir, "bias_coefficients_per_fold_repeat_adni.csv")
df_bias.to_csv(coef_csv, index=False)
print(f"Saved bias coefs CSV: {coef_csv}")

metrics_txt = os.path.join(results_dir, "final_metrics_raw_vs_corrected.txt")
with open(metrics_txt, "w") as f:
    f.write("FINAL METRICS (RAW)\n")
    f.write(f"MAE  {np.mean(mae_raw_list):.4f} ± {np.std(mae_raw_list):.4f}\n")
    f.write(f"RMSE {np.mean(rmse_raw_list):.4f} ± {np.std(rmse_raw_list):.4f}\n")
    f.write(f"R2   {np.mean(r2_raw_list):.4f} ± {np.std(r2_raw_list):.4f}\n\n")
    f.write("FINAL METRICS (BIAS-CORRECTED)\n")
    f.write(f"MAE  {np.mean(mae_cor_list):.4f} ± {np.std(mae_cor_list):.4f}\n")
    f.write(f"RMSE {np.mean(rmse_cor_list):.4f} ± {np.std(rmse_cor_list):.4f}\n")
    f.write(f"R2   {np.mean(r2_cor_list):.4f} ± {np.std(r2_cor_list):.4f}\n")

print(f"Saved metrics TXT: {metrics_txt}")


plt.figure(figsize=(8, 6))
plt.scatter(dfp["Real_Age"], dfp["Predicted_Age_corrected_global"],
            alpha=0.7, edgecolors="k")
mn = float(min(dfp["Real_Age"].min(), dfp["Predicted_Age_corrected_global"].min()))
mx = float(max(dfp["Real_Age"].max(), dfp["Predicted_Age_corrected_global"].max()))
plt.plot([mn, mx], [mn, mx], color="red", linestyle="dashed")
plt.ylabel("Predicted Age (GLOBAL OOF bias-corrected)")
plt.title("Predicted vs Real (Bias-corrected, all folds×repeats)")
plt.grid(True)
plt.tight_layout()

pred_path = os.path.join(results_dir, "predicted_vs_real_bias_corrected.png")
plt.savefig(pred_path, dpi=300, bbox_inches="tight")
plt.show()
plt.close()
print(f"Saved corrected predicted-vs-real plot to: {pred_path}")
...
# ==========================
# SAVE FINAL MODEL + BIAS-CORRECTION WRAPPER
# ==========================
bias = torch.load(bias_path, map_location="cpu")
final_model_bc = BrainAgeGATv2_BiasCorrected(
    base_model=final_model,
    slope=bias["slope_global"],
    intercept=bias["intercept_global"],
).to(device)

model_name_bc = "brainage_adni1_prediction_BIAS_CORRECTED_model.pt"
model_path_bc = os.path.join(results_dir, model_name_bc)

torch.save({
    "model_state_dict": final_model_bc.state_dict(),
    "bias_params": bias,
    "epochs": epochs,
    "lr": 0.002,
    "weight_decay": 1e-4
...
print(f"\nFinal ADNI model (BIAS-CORRECTED) saved as: {model_path_bc}")


# Save model after full training
model_name = "brainage_adni1_prediction_model.pt"
...
print(f"\nFinal ADNI model saved as: {model_path}")


# ==========================
# Bias correlation CHECK (SUBJECT-LEVEL, GLOBAL OOF)
# ==========================

# df_subj ya lo tienes arriba (Age, Pred_raw, BAG, cBAG_global, Pred_corr_global)
# así que SOLO imprime estas correlaciones:

corr_bag_subj = df_subj["Age"].corr(df_subj["BAG"])
corr_cbag_subj = df_subj["Age"].corr(df_subj["cBAG_global"])

print("\n=== SUBJECT-LEVEL bias check (GLOBAL OOF) ===")
print(f"corr(Age, BAG)         = {corr_bag_subj:+.3f}")
print(f"corr(Age, cBAG_global) = {corr_cbag_subj:+.3f}")



model_name_raw = "brainage_adni1_prediction_model_RAW.pt"
model_path_raw = os.path.join(results_dir, model_name_raw)

torch.save({
    "model_state_dict": final_model.state_dict(),
    "model_architecture": "BrainAgeGATv2",
    "global_feat_dim": 16,
    "epochs": epochs,
    "lr": 0.002,
    "weight_decay": 1e-4
...
print(f"\nFinal ADNI RAW model saved as: {model_path_raw}")


import json

bias_path = os.path.join(results_dir, "bias_correction_global.json")
