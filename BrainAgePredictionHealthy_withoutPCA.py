...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 26 10:40:30 2026
...
OUT_DIR = os.path.join(os.environ["WORK"], "ines/results/BrainAgePrediction_withoutPCA")
...
metadata_ids = set(df_matched_addecode_healthy["MRI_Exam_fixed"])
connectome_ids = set(matched_connectomes_healthy_addecode.keys())

final_overlap = fa_md_vol_ids & metadata_ids & connectome_ids

print(" Subjects with FA/MD/Vol + Connectome:", len(final_overlap))
...
in_metadata = example_id in df_matched_addecode_healthy["MRI_Exam_fixed"].values
...
addecode_healthy_metadata["Clustering_Coeff"] = np.nan
addecode_healthy_metadata["Path_Length"] = np.nan
...
for subject, matrix_log in log_thresholded_connectomes.items():
    try:
        ...
        addecode_healthy_metadata.loc[
            addecode_healthy_metadata["MRI_Exam_fixed"] == subject, "Clustering_Coeff"
        ] = clustering
        addecode_healthy_metadata.loc[
            addecode_healthy_metadata["MRI_Exam_fixed"] == subject, "Path_Length"
        ] = path
...
# ===============================
# Step 11: Normalize Metadata  Columns
# ===============================
...
#label encoding sex
le_sex = LabelEncoder()
addecode_healthy_metadata["sex_encoded"] = le_sex.fit_transform(addecode_healthy_metadata["sex"].astype(str))


# --- Label encode genotype ---
le = LabelEncoder()
addecode_healthy_metadata["genotype"] = le.fit_transform(addecode_healthy_metadata["genotype"].astype(str))

# --- Normalize numerical columns ---
numerical_cols = ["Systolic", "Diastolic", "Clustering_Coeff", "Path_Length"]


addecode_healthy_metadata[numerical_cols] = addecode_healthy_metadata[numerical_cols].apply(zscore)
...
subject_to_demographic_tensor = {
    ...
    for _, row in addecode_healthy_metadata.iterrows()
}
...
for subject, matrix_log in log_thresholded_connectomes.items():
    try:
        ...
        # === Get target age
        age_row = addecode_healthy_metadata.loc[
            addecode_healthy_metadata["MRI_Exam_fixed"] == subject, "age"
        ]
        if age_row.empty:
            continue
        age = torch.tensor([age_row.values[0]], dtype=torch.float)

        # === Concatenate global features (demographics + graph metrics)
        ...
        data = Data(
            ...
            edge_attr=edge_attr,
            y=age,
            global_features=global_feat.unsqueeze(0)  # Shape: (1, 6)
        )
...
# === Report stats ===
print()
expected = set(subject_to_demographic_tensor.keys())
...
example_subject = list(subject_to_demographic_tensor.keys())[0]
...
class BrainAgeGATv2(nn.Module):
    def __init__(self, global_feat_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(128 + 32, 128),  # 128 from GNN output + 64 from metadata branches
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(self, data):
        ...
        global_embed = torch.cat([meta_embed, graph_embed], dim=1)  # Shape: [batch_size, 64]
...
df_filtered = addecode_healthy_metadata[
    addecode_healthy_metadata["MRI_Exam_fixed"].isin(graph_subject_ids)
].copy()
...
for repeat in range(repeats):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(graph_data_list_addecode, age_bins)):
        ...
        model = BrainAgeGATv2(global_feat_dim=6).to(device)
        ...
        for epoch in range(epochs):
            ...
            if test_loss < best_loss:
                ...
                ckpt = os.path.join(CKPT_DIR, f"model_rep_{repeat+1}_fold_{fold+1}.pt")
...
csv_path = os.path.join(TABLES_DIR, "cv_predictions_addecode.csv")
df_preds_addecode.to_csv(csv_path, index=False)
print(f"CSV saved to: {csv_path}")

df_bias_coefs = pd.DataFrame(coef_rows)
coef_path = os.path.join(TABLES_DIR, "bias_coefficients_per_fold_repeat.csv")
...
# Initialize model
final_model = BrainAgeGATv2(global_feat_dim=6).to(device)
...
train_path = os.path.join(PLOTS_DIR, "cv_train_loss_mean_95CI.png")
...
final_model_path = os.path.join(MODELS_DIR, "model_trained_on_all_healthy.pt")
...
plt.grid(True)
plt.tight_layout()
loss_plot_path = os.path.join(PLOTS_DIR, "final_model_loss_curve.png")
...
csv_path = os.path.join(TABLES_DIR, "final_model_loss_history.csv")
